...
def removeInventory(_conn,_i_storeID):
    try:   

        sql = """DELETE FROM Inventory WHERE i_storeID = '{}';""".format(_i_storeID)
        cur = _conn.cursor()
        cur.execute(sql)
            ...
            if selection =='2': 
                while True: 
                    ...
                    if selection =='1': 
                        ...
                        ID=input("Enter storeID to remove:")
                        removeInventory(conn,ID)
